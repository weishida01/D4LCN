...
def test_kitti_3d(dataset_test, test_split, net, rpn_conf, results_path, test_path, use_log=True):
    ...
    imlist = list_files(os.path.join(test_path, test_split, 'image', ''), '*.png')
    ...
    for imind, impath in enumerate(imlist):
        ...
        if rpn_conf.depth_channel == 3:
            depth = cv2.imread(impath.replace('image', 'depth'))
        else:
            depth = cv2.imread(impath.replace('image', 'depth'), cv2.IMREAD_UNCHANGED)
            depth = depth[:, :, np.newaxis]
            if rpn_conf.use_seg:
                seg = cv2.imread(impath.replace('image', 'seg'), cv2.IMREAD_UNCHANGED)
                seg = seg[:, :, np.newaxis]
        ...
        # read in calib
        p2 = read_kitti_cal(os.path.join(test_path, test_split, 'calib', name + '.txt'))  # 3d to 2d
